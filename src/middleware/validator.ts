...
/**
 * Helper to create a Joi object schema for a single numeric path parameter.
 * Usage: paramSchema('restaurantId') -> Joi.object({ restaurantId: Joi.number().integer().min(1).required() })
 */
export const paramSchema = (name: string) =>
  Joi.object({
    [name]: Joi.number().integer().min(1).required(),
  });
